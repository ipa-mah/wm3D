...
struct CameraParameters
{
	...
	CameraParameters(float fx, float fy, float cx, float cy, int height, int width) : focal_x(fx), focal_y(fy), c_x(cx), c_y(cy), image_height(height), image_width(width)
	{
	}
	CameraParameters(float fx, float fy, float cx, float cy) : focal_x(fx), focal_y(fy), c_x(cx), c_y(cy)
	{
	}
	...
	cv::Mat cvMatMatrix()
	{
		return (cv::Mat_<float>(3, 3) << this->focal_x, 0.0f, this->c_x, 0.0f, this->focal_y, this->c_y, 0.0f, 0.0f, 1.0f);
	}
};
...
struct TsdfVolume
{
  public:
	...
	TsdfVolume(short2* data, int3 dims, float3 voxel_size, float trunc_dist, float max_weight);
	TsdfVolume(const TsdfVolume&);
	__device__ __forceinline__ short2* operator()(int x, int y, int z);
	__device__ __forceinline__ const short2* operator()(int x, int y, int z) const;
	__device__ __forceinline__ short2* beg(int x, int y) const;
	__device__ __forceinline__ short2* zstep(short2* const ptr) const;
