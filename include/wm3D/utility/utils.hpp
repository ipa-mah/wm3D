...
static std::string getFileName(const std::string& full_path, bool with_ext)
{
	...
	if (!with_ext) filename = filename.substr(0, filename.length() - 4);  // remove ext
	return filename;
}
...
static void readIntrinsicsAndNumViews(const std::string data_path, Eigen::Matrix3d& cam_params, int& num_views, int& image_width, int& image_height, float& depth_scale)
{
	const std::string info_file = data_path + "/info.txt";
	...
	std::cout << "number of frames: " << num_views << std::endl;
	std::cout << "intrinsics: " << std::endl << cam_params << std::endl;
}
