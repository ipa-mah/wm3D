...
class TSDFVolumeDevice
{
  ...
  public:
	__DEVICE__ void integrate(const Eigen::Vector3i& voxel_idx, const PtrStepSz<unsigned short>& depth_image, const CameraIntrinsicCuda& cam_params, const Eigen::Matrix4f& world_to_cam,
							  const float depth_scale);
	__DEVICE__ Eigen::Vector3f rayCasting(const Eigen::Vector2i& p, const CameraIntrinsicCuda& cam_params, const Eigen::Matrix4f& cam_to_world, const float ray_step);

	__DEVICE__ 
  public:
	friend class TestTSDFVolume;
};

class TestTSDFVolume
{
  public:
	std::shared_ptr<TSDFVolumeDevice> device_ = nullptr;
  ...
  public:
	TestTSDFVolume();
	TestTSDFVolume(int res, float voxel_length, float sdf_trunc, Eigen::Matrix4d& volume_to_world);
	void create(int res);
	void release();
	void updateDevice();
...
class TSDFVolumeCuda
{
  ...
  public:
	...
	void rayCasting(DeviceArray2D<float3>& model_vertex, DeviceArray2D<float3>& model_normal, const CameraIntrinsicCuda& cam_params, const Eigen::Matrix4f& cam_to_world, const float ray_step);
};
