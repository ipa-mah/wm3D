...
void TSDFVolumeCuda::create(const Eigen::Vector3i& dims)
{
	...
	CheckCuda(cudaMalloc(&(device_->weight_), sizeof(float) * NNN));
...
{
	if (device_ != nullptr) device_->dims_ = dims_;
...
void TSDFVolumeCuda::reset()
{
	...
	{
		const size_t NNN = dims_(0) * dims_(1) * dims_(2);
		// CheckCuda(cudaMemset(device_->tsdf_,1, sizeof(float) * NNN));
		// CheckCuda(cudaMemset(device_->color_,0, sizeof(uchar3) * NNN));
		// CheckCuda(cudaMemset(device_->weight_,5.0, sizeof(float) * NNN));
		TSDFVolumeCudaKernel::reset(*this);
	}
}

void TSDFVolumeCuda::uploadVolume(std::vector<float>& tsdf, std::vector<float>& weight, std::vector<uchar3>& color)
{
	...
	CheckCuda(cudaMemcpy(device_->weight_, weight.data(), sizeof(float) * NNN, cudaMemcpyHostToDevice));
}

std::tuple<std::vector<float>, std::vector<float>, std::vector<uchar3>> TSDFVolumeCuda::downloadVolume()
{
	assert(device_ != nullptr);
	...
	CheckCuda(cudaMemcpy(weight.data(), device_->weight_, sizeof(float) * NNN, cudaMemcpyDeviceToHost));
	CheckCuda(cudaMemcpy(color.data(), device_->color_, sizeof(uchar3) * NNN, cudaMemcpyDeviceToHost));

	return std::make_tuple(std::move(tsdf), std::move(weight), std::move(color));
}

void TSDFVolumeCuda::integrate(const DeviceArray2D<uchar3>& color_image, const DeviceArray2D<ushort>& depth_image, const CameraIntrinsicCuda& intrins, const Eigen::Matrix4f& world_to_cam,
