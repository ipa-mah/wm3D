...
namespace cuda
{
__global__ void initializeVolumeKernel(PtrStepSz<float> tsdf_volume, PtrStepSz<float> weight_volume, const int res)
{
...
__global__ void integrateKernel(const PtrStepSz<unsigned short> depth_image, PtrStepSz<float> tsdf_volume, PtrStepSz<float> weight_volume, const int res, float voxel_length, const float depth_scale,
								const CameraIntrinsicCuda cam_params, const float truncation_distance, const Eigen::Matrix4f world_to_cam)
{
...
__device__ __forceinline__ float trilinearInterpolation(const Eigen::Vector3f& point, const PtrStepSz<float>& volume, const int& volume_size)
{
	int3 point_in_grid = make_int3(__float2int_rn(point(0)), __float2int_rn(point(1)), __float2int_rn(point(2)));
...
__device__ __forceinline__ void getMaxMin(const float volume_max, const Eigen::Vector3f& origin, const Eigen::Vector3f& ray_dir, float& max_range, float& min_range)
{
...
__global__ void kernelRayCasting(const PtrStepSz<float> tsdf_volume, PtrStepSz<float3> model_vertex, PtrStepSz<float3> model_normal, const int volume_size, const float voxel_size,
								 const CameraIntrinsicCuda intrins, const float truncation_distance, const float ray_step, const Eigen::Matrix3f cam_to_world_rot,
								 const Eigen::Vector3f cam_to_world_trans)
{
	const int x = blockIdx.x * blockDim.x + threadIdx.x;
	const int y = blockIdx.y * blockDim.y + threadIdx.y;

	if (x >= model_vertex.cols || y >= model_vertex.rows) return;
	...
	const Eigen::Vector3f ray_dir_cam = Eigen::Vector3f((x - intrins.cx_) / intrins.fx_, (y - intrins.cy_) / intrins.fy_, 1.f);
	...
	if (ray_length >= max_range) return;
	ray_length += voxel_size;
	Eigen::Vector3f grid = (cam_to_world_trans + (ray_dir * ray_length)) / voxel_size;

	float tsdf = tsdf_volume.ptr(__float2int_rd(grid(2)) * volume_size + __float2int_rd(grid(1)))[__float2int_rd(grid(0))];
	...
	for (; ray_length < max_search_length; ray_length += truncation_distance * ray_step)
	{
		...
		if (grid(0) < 1 || grid(0) >= volume_size - 1 || grid(1) < 1 || grid(1) >= volume_size - 1 || grid(2) < 1 || grid(2) >= volume_size - 1) continue;

		const float previous_tsdf = tsdf;
		tsdf = tsdf_volume.ptr(__float2int_rd(grid(2)) * volume_size + __float2int_rd(grid(1)))[__float2int_rd(grid(0))];

		if (previous_tsdf < 0.f && tsdf > 0.f)  // Zero crossing from behind
			break;
		if (previous_tsdf > 0.f && tsdf < 0.f)
		{  // Zero crossing
			const float t_star = ray_length - truncation_distance * ray_step * previous_tsdf / (tsdf - previous_tsdf);

			const Eigen::Vector3f vertex = cam_to_world_trans + ray_dir * t_star;

			const Eigen::Vector3f location_in_grid = (vertex / voxel_size);
			if (location_in_grid(0) < 1 | location_in_grid(0) >= volume_size - 1 || location_in_grid(1) < 1 || location_in_grid(1) >= volume_size - 1 || location_in_grid(2) < 1 ||
				location_in_grid(2) >= volume_size - 1)
				break;
			// Compute normal
			Eigen::Vector3f normal, shifted;
			shifted = location_in_grid;
			shifted(0) += 1;
			if (shifted(0) >= volume_size - 1) break;
			...
			shifted = location_in_grid;
			shifted(1) += 1;
			if (shifted(1) >= volume_size - 1) break;
			...
			shifted = location_in_grid;
			shifted(2) += 1;
			if (shifted(2) >= volume_size - 1) break;
...
void TSDFVolumeCuda::rayCasting(DeviceArray2D<float3>& model_vertex, DeviceArray2D<float3>& model_normal, const CameraIntrinsicCuda& intrins, const Eigen::Matrix4f& cam_to_world, const float ray_step)
{
	...
	const dim3 blocks(DIV_CEILING(intrins.width_, THREAD_2D_UNIT), DIV_CEILING(intrins.height_, THREAD_2D_UNIT));
	const dim3 threads(THREAD_2D_UNIT, THREAD_2D_UNIT);

	kernelRayCasting<<<blocks, threads>>>(tsdf_volume_, model_vertex, model_normal, res_, voxel_length_, intrins, sdf_trunc_, ray_step, cam_to_world_rot, cam_to_world_trans);
...
class AddDevice
{
  public:
	...
	__device__ __forceinline__ void operator()(int* vector_add, const int n) const
	{
...
__device__ void TSDFVolumeDevice::integrate(const Eigen::Vector3i& voxel_idx, const PtrStepSz<unsigned short>& depth_image, const CameraIntrinsicCuda& cam_params, const Eigen::Matrix4f& world_to_cam,
											const float depth_scale)
{
	// Convert to voxel grid to global coordinate
	const float3 global_voxel =
		make_float3((static_cast<float>(voxel_idx(0)) + 0.5f) * voxel_length_, (static_cast<float>(voxel_idx(1)) + 0.5f) * voxel_length_, (static_cast<float>(voxel_idx(2)) + 0.5f) * voxel_length_);
	// convert voxel from global to local camera coordinate
	const Eigen::Vector3f camera_voxel = (world_to_cam * Eigen::Vector4f(global_voxel.x, global_voxel.y, global_voxel.z, 1.0)).head<3>();
	if (camera_voxel(2) <= 0) return;
	// projection
...
void TestTSDFVolume::initializeVolume()
{
	const int num_blocks_x = DIV_CEILING(res_, THREAD_3D_UNIT);
	const int num_blocks_y = DIV_CEILING(res_, THREAD_3D_UNIT);
	const int num_blocks_z = DIV_CEILING(res_, THREAD_3D_UNIT);
	const dim3 blocks(num_blocks_x, num_blocks_y, num_blocks_z);
	const dim3 threads(THREAD_3D_UNIT, THREAD_3D_UNIT, THREAD_3D_UNIT);
	initializeVolumeKernel<<<blocks, threads>>>(device_->tsdf_volume_, device_->weight_volume_, device_->res_);
	CheckCuda(cudaDeviceSynchronize());
	CheckCuda(cudaGetLastError());
}
