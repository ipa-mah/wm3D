...
namespace cuda
{
									...
									const float depth_scale, const CameraIntrinsicCuda cam_params, const float truncation_distance, const Eigen::Matrix4f world_to_cam)
	{
		...
		//printf("%f %f %f %f \n ",truncation_distance,sdf,depth,camera_voxel(2));
							  ...
							  const float depth_scale)
	{
		...
		integrateKernel<<<blocks, threads>>>(depth_map, tsdf_volume_,
											weight_volume_, dims_, voxel_length_,
						  					depth_scale, cam_params,
						  					sdf_trunc_, world_to_cam);
		CheckCuda(cudaDeviceSynchronize());
		CheckCuda(cudaGetLastError());
	}
