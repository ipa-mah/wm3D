...
class PoolDispatcher {
 ...
 private:
  ...
  template <std::size_t Index>
      typename std::enable_if < Index<NumPools, void*>::type dispatchAllocate(
                                    std::size_t const& requestSize) {
  ...
  template <std::size_t Index>
      ...
      Index<NumPools>::type dispatchDeallocate(
          void* p, std::size_t const& requestSize) noexcept {
    if (requestSize <= std::get<Index>(pools_).CellSize) {
  ...
  template <std::size_t Index>
  typename std::enable_if<!(Index < NumPools)>::type dispatchDeallocate(
      void*, std::size_t const&) noexcept {
...
template <std::size_t NumPools>
thread_local
    typename detail::Pools<NumPools>::type PoolDispatcher<NumPools>::pools_;
}  // namespace lfpAlloc
