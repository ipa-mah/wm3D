...
void StichObjs(tinyobj::attrib_t& out_attribute,
               std::vector<tinyobj::shape_t>& out_shape,
               std::vector<tinyobj::material_t>& out_material,
               const std::vector<Attribute>& attributes,
               const std::vector<Shape>& shapes,
               const std::vector<Material>& materials) {
  // The amount of attributes, shape-vectors and material-vecotrs should be the
  // same.
  if (attributes.size() != shapes.size() &&
      attributes.size() != materials.size()) {
    std::cerr << "Size of attributes, shapes and Materials don't fit!"
              << attributes.size() << " " << shapes.size() << " "
  ...
  for (size_t i = 0; i < shapes.size(); i++) {
    ...
    // Copy attributes (3 floats per vertex, 3 floats per normal, 2 floats per
    // texture-coordinate, 3 floats per color) You could also include a check
    // here, if the sizes are dividable by 3 (resp. 2), but it's safe to simply
    // assume, they do.
    std::copy(attributes[i].vertices.begin(), attributes[i].vertices.end(),
              out_attribute.vertices.begin() + vertex_idx_offset * 3);
    vertex_idx_offset += attributes[i].vertices.size() / 3;
    std::copy(attributes[i].normals.begin(), attributes[i].normals.end(),
              out_attribute.normals.begin() + normal_idx_offset * 3);
    normal_idx_offset += attributes[i].normals.size() / 3;
    std::copy(attributes[i].texcoords.begin(), attributes[i].texcoords.end(),
              out_attribute.texcoords.begin() + texcoord_idx_offset * 2);
    texcoord_idx_offset += attributes[i].texcoords.size() / 2;
    std::copy(attributes[i].colors.begin(), attributes[i].colors.end(),
              out_attribute.colors.begin() + color_idx_offset);
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < num_objfiles; i++) {
    ...
    std::string warn;
    std::string err;
    bool ret = tinyobj::LoadObj(&attributes[i], &shapes[i], &materials[i],
                                &warn, &err, argv[i + 1]);
    if (!warn.empty()) {
  ...
  StichObjs(out_attribute, out_shape, out_material, attributes, shapes,
            materials);

  bool coordTransform = true;
  bool ret = WriteObj(out_filename, out_attribute, out_shape, out_material,
                      coordTransform);
  assert(ret);
