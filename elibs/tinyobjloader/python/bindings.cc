...
PYBIND11_MODULE(tinyobjloader, tobj_module) {
  ...
  py::class_<mesh_t>(tobj_module, "mesh_t")
      ...
      .def("numpy_indices", [](mesh_t &instance) {
        ...
        // normal_index, texcoord_index). numpy_indices = [0, -1, -1, 1, -1, -1,
        // ...] C++11 or later should pack POD struct tightly and does not
        // reorder variables, so we can memcpy to copy data. Still, we check the
        // size of struct and byte offsets of each variable just for sure.
